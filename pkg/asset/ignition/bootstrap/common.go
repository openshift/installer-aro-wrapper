...
import (
	...
	aro "github.com/openshift/installer-aro-wrapper/pkg/asset/ignition"
	"github.com/openshift/installer-aro-wrapper/pkg/asset/machines"
...
func (a *Common) getTemplateData(dependencies asset.Parents, bootstrapInPlace bool) *bootstrapTemplateData {
	...
	registries := []sysregistriesv2.Registry{}
	for _, group := range bootstrap.MergedMirrorSets(installConfig.Config.ImageDigestSources) {
...
func (a *Common) addParentFiles(dependencies asset.Parents) {
	...
	for _, asset := range []asset.WritableAsset{
		...
		&mcign.MasterIgnitionCustomizations{},
		&mcign.WorkerIgnitionCustomizations{},
		&bootstrap.CVOIgnore{}, // this must come after manifests.Manifests so that it replaces cvo-overrides.yaml
	} {
